...
void roundrobin(int n,int tq,int st[],int bt[]) {
    ...
    while(1) {
        for(i=0,count=0;i<n;i++){
            temp1=tq;
            if(st[i]==0) {
            // when service time of a process equals zero then
        ...
        if(n==count)// it indicates all processes have completed their taskbecause the count value
            break;// incremented when service timeequals 0
    } //end of while
    ...
    printf("Process_noBurst timeWait timeTurn around time\n");
    for(i=0;i<n;i++)
        printf("%d\t\t%d\t\t%d\t\t%d\n",i+1,bt[i],wt[i],tat[i]);
...
void srtf(int n) {
    ...
    for(j = i + 1; j <= n; j++)
    {
    if(process[i].arrival > process[j].arrival)    // sort arrival time of aprocess
    {
    temp = process[i];
    process[i] = process[j];
    process[j] = temp;
